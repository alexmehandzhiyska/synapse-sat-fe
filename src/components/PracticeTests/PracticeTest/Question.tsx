...
const Question = ({ question, sectionName, selectedChoiceId, onSelect }: QuestionProps) => {
    const isReadingWriting = sectionName === 'reading_writing';

    return (
        <div className={isReadingWriting ? 'grid h-full lg:grid-cols-2' : 'h-full'}>
            {isReadingWriting && (
            ...
            <div className="h-full overflow-y-auto bg-white p-8 lg:p-12">
                <div className="mx-auto max-w-2xl">
                    <p
                        className={`mb-6 ${isReadingWriting ? 'text-lg' : 'text-xl'} font-semibold leading-8 text-[#13385A]`}
                    >
                        {question.prompt}
                    </p>
                    <AnswerChoices
